#!/usr/bin/env python3
"""
Analyse du Handler 'a' (Index 1) @ 0x00431A20
1 occurrence trouvée dans le dataset
"""
from capstone import *
import struct

EXE_FILE = "DOCS/europeo.exe"
HANDLER_A_ADDR = 0x00431A20
...
def main():
    print("="*80)
    print("ANALYSE HANDLER 'a' (Index 1) @ 0x00431A20")
    print("="*80)
    print()
    ...
    offset = va_to_file_offset(HANDLER_A_ADDR, sections)
    if not offset:
        print("✗ Impossible de trouver l'adresse")
        return

    print(f"Handler @ 0x{HANDLER_A_ADDR:08X}")
    ...
    for i, insn in enumerate(md.disasm(code, HANDLER_A_ADDR)):
        if i >= 50:
            break
    ...
    print("-" * 80)
    print("Handler 'e' (Audio+Image) appelle: 0x427B56 → 0x4321B6")
    ...
    known_funcs = ['0x4268f8', '0x427b56', '0x427d34', '0x427fae', '0x4280ea', '0x4321b6']
