#!/usr/bin/env python3
"""
Extract complete assembly code for ALL TVN methods
One markdown file per TVN structure
Alternative to Ghidra - uses radare2 for disassembly
"""
...
def disassemble_function_r2(binary_path, func_addr, max_instructions=500):
    """Disassemble a function using radare2"""

    # Run radare2 to analyze and disassemble
    cmd = [
        'r2',
        '-q',  # Quiet
        '-A',  # Analyze
        '-c', f'pdf @ {func_addr}',  # Print disassembly of function
        binary_path
    ]
...
def extract_strings_from_asm(asm_code):
    ...
    # Look for string patterns in radare2 output
    # Typically: str.Something or "quoted string"
    patterns = [
        r'str\.([^\s,\]]+)',  # str.NAME
        r'"([^"]+)"',          # "string"
        r'0x[0-9a-f]+.*; "(.*?)"',  # address ; "string"
    ]
...
def extract_function_calls(asm_code):
    ...
    # Format: call fcn.00401234 or call sym.FunctionName
    pattern = r'call\s+(fcn\.[0-9a-f]+|sym\.[^\s;]+|[0-9a-fx]+)'
...
def identify_important_calls(calls):
    ...
    for call in calls:
        ...
        if any(keyword in call_lower for keyword in ['getstring', 'getint', 'profile', 'tprofile', 'loadfrom', 'parse']):
...
def extract_method_complete(binary_path, method, method_index, output_file):
    ...
    asm_code = disassemble_function_r2(binary_path, method['address'])

    if "ERROR" in asm_code:
        f.write(f"⚠️ **Error disassembling**: {asm_code}\n\n")
        f.write("---\n\n")
        return
    ...
    if calls:
        ...
        if other:
            f.write("**Other Calls**:\n\n")
            for call in other[:20]:  # Limit to first 20
                f.write(f"- `{call}`\n")
            if len(other) > 20:
...
def extract_tvn_structure(binary_path, struct_name, vtable_va, output_dir):
    ...
    with open(output_file, 'w') as f:
        ...
        for i, method in enumerate(methods):
            extract_method_complete(binary_path, method, i, f)
...
def extract_all_tvn_structures(binary_path, output_dir):
    ...
    for struct_name, vtable_va in TVN_VTABLES.items():
        ...
        extract_tvn_structure(binary_path, struct_name, vtable_va, output_dir)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: extract_all_tvn_asm.py <europeo.exe> [output_dir]")
        print()
        print("This script extracts complete assembly code for all TVN methods.")
        print("One markdown file is created per TVN structure.")
        print()
        print("Example:")
        print("  python3 extract_all_tvn_asm.py DOCS/europeo.exe TVN_ASM_EXTRACTS")
        sys.exit(1)
