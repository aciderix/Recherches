#!/usr/bin/env python3
"""
Analyse du Handler 'g' (Index 7) @ 0x00431B2B
Trouvé dans danem.vnd: "runprj couleurs1.vnp 54g"
"""
from capstone import *
import struct

EXE_FILE = "DOCS/europeo.exe"
HANDLER_G_ADDR = 0x00431B2B
...
def main():
    print("="*80)
    print("ANALYSE HANDLER 'g' (Index 7) @ 0x00431B2B")
    print("="*80)
    print()
    ...
    offset = va_to_file_offset(HANDLER_G_ADDR, sections)
    if not offset:
        print("✗ Impossible de trouver l'adresse")
        return

    print(f"Handler @ 0x{HANDLER_G_ADDR:08X}")
    ...
    for i, insn in enumerate(md.disasm(code, HANDLER_G_ADDR)):
        if i >= 50:
            break
