#!/usr/bin/env python3
"""
Analyse du Handler 'c' (Index 3) @ 0x00431881
"""
from capstone import *
import struct

EXE_FILE = "DOCS/europeo.exe"
HANDLER_C_ADDR = 0x00431881
...
def main():
    print("="*80)
    print("ANALYSE HANDLER 'c' (Index 3) @ 0x00431881")
    print("="*80)
    print()
    ...
    offset = va_to_file_offset(HANDLER_C_ADDR, sections)
    if not offset:
        print("✗ Impossible de trouver l'adresse")
        return

    print(f"Handler @ 0x{HANDLER_C_ADDR:08X}")
    ...
    print("DÉSASSEMBLAGE (40 premières instructions):")
    print("-" * 80)

    calls_found = []
    jumps_found = []

    for i, insn in enumerate(md.disasm(code, HANDLER_C_ADDR)):
        if i >= 40:
            break
